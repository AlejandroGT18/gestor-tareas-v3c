  ...
  tasks.forEach((task) => {
    ...
    if (editingTaskId === task.id) {
      li.innerHTML = `
        ...
        task.text
      )}" class="input-field" style="margin-bottom:8px;max-width:220px;">
                            <input type="datetime-local" id="edit-date-${
                              task.id
                            }" value="${
        task.datetime || ""
      }" class="input-field" style="max-width:180px;">
                            <input type="text" id="edit-categoria-${
                              task.id
                            }" value="${escapeHtml(
        task.categoria
      )}" placeholder="Categoría" class="input-field" style="max-width:180px;">
                        </div>
                    </div>
                    <div class="task-actions">
                        <button class="btn-success" onclick="saveTask(${
                          task.id
                        })">Guardar</button>
      ...
      li.innerHTML = `
                <div class="task-content ${taskClass}">
                    ...
                    <div class="task-actions">
                        <button class="${buttonClass}" onclick="toggleTaskComplete(${
        task.id
      })">${buttonText}</button>
...
async function saveTask(taskId) {
  ...
  const newCategoria = document
    .getElementById(`edit-categoria-${taskId}`)
    .value.trim();
  if (!newText) {
...
async function toggleTaskComplete(taskId) {
  try {
    ...
    if (!response.ok)
      throw new Error(result.error || "Error al cambiar estado de tarea");
